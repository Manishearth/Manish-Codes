...
/*
a=[];i=0;$('.keyblock').each(function(){
a[i]={};
a[i].letter=$(this).find('label').html();
a[i].key=a[i].letter;
a[i].chars=[];j=0;
$(this).find('button').each(function(){
a[i].chars[j]={html:this.innerHTML}
$('iframe#typeit_textbox_iframe')[0].contentDocument.body.innerHTML="";
this.click();
a[i].chars[j].inserter=$('iframe#typeit_textbox_iframe')[0].contentDocument.body.innerHTML;
a[i].chars[j].title=this.title.split("\(")[0];
a[i].chars[j].name=this.name
 ...
 IPAKeyboard.makeFloat=function(tid){
     ...
     for(var i=0;i<IPAKeyboard.data.length;i++){
         ...
         for(var j=0;j<keyset.chars.length;j++){
             el2=$('<span class=IPAKey name="'+keyset.chars[j].name+'" title="'+keyset.chars[j].name+'">'+keyset.chars[j].html+'</span>').on('click',IPAKeyboard.clickButtonEventLambda("",keyset.chars[j].inserter,tid));
             el2.appendTo(el);
             ctr++;
         }
...
//with_jquery(function($) {
 IPAKeyboard.addButton('IPA',  IPAKeyboard.makeFloat ,'ipakeyboard','','Open IPA Keyboard',false)
//});
